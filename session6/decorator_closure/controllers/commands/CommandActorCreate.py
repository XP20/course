...
class CommandActorCreate(ICommand):
    ...
    def execute(self):
        ...
        ControllerGame.instance().add_actor_controller(self.actor_controller)

    def undo(self):
        actor_controllers = ControllerGame.instance().actor_controllers
        if self.actor in self.game.actors:
            self.game.actors.remove(self.actor)
        if self.actor_controller in actor_controllers:
            ControllerGame.instance().remove_actor_controller(self.actor_controller)
